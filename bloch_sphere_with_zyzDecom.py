...
def zyz_decomposition(U: np.ndarray):
    """
    Decompose a 2x2 unitary into:
    ...
    # rompute beta
    beta = 2 * np.arccos(np.clip(np.abs(a), -1.0, 1.0))

    # edge case
    if np.isclose(np.sin(beta / 2), 0):
        alpha = 0.0
...
class BlochSphereSimulator:
    ...
    def show_unitary_info(self, U, name="U"):
        axis, theta = unitary_to_axis_angle(U)
        print(f"\n--- {name} Unitary Interpretation ---")
        print(f"Rotation angle: {theta:.3f} radians")
        print(f"Rotation axis: ({axis[0]:.3f}, {axis[1]:.3f}, {axis[2]:.3f})")

        alpha, beta, gamma = zyz_decomposition(U)

        print(f"\n--- {name} ZYZ Decomposition ---")
        print(f"RZ({alpha:.3f}) -> RY({beta:.3f}) -> RZ({gamma:.3f})")

    def apply_gate(self, gate_name: str, theta: float = None, show_decomp=False):
        """This applies gates"""
...
def gate_loop(sim):
    while True:
        ...
        # rotation gates like rx(pi/2)
            ...
            try:
                name = gate[:2].upper()  # RX / RY / RZ
                angle_str = gate[gate.find("(")+1 : gate.find(")")]
                
                theta = eval(angle_str, {"pi": np.pi})

                sim.apply_gate(name, theta, show_decomp=True)

            except Exception as e:
                print("Invalid rotation format. Use rx(pi/2).")
                print("Debug:", e)
                continue
